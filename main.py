    ...
    if db['hitPriceTarget'] not in range(
        ...
        if len(
                checkPriceTrend(db['hitPriceTarget'], current_price,
                                priceTargets)) != 0:
            if db['noti'] != checkPriceTrend(db['hitPriceTarget'],
            ...
            if db['hitPriceTarget'] > current_price:
                db["noti"] = reverse_alert(db['hitPriceTarget'], current_price,
                                           priceTargets)
...
async def on_message(message):
    ...
    if message.content.startswith('$set '):
        ...
        if isCryptoSupported(cryptoConcerned) and check(priceTargets):
            ...
            await message.channel.send(
                f'Successfully set price alert for {db["detect crypto"]} at {list(db["detect price"])} USD.'
            )

        else:
            await message.channel.send(
                f'Unsuccessful setting of price alerts. Please try again.')
...
BOT_TOKEN = 'ODgwMDI4NjI3NzkyNzE5OTAy.YSYUhw.lqHGRadZEdP_arxx_X_704T48Oc'
